...
function FieldsLinker(selector) {
    ...
    this.draw = function () {
        ...
        links.forEach(function (item, i) {
            ...
            var positionA = self.listA.map(function (e) { return e.id; }).indexOf(item.from);
            var positionB = self.listB.map(function (e) { return e.id; }).indexOf(item.to);
    ...
    this.makeDropDownForLists = function () {
        var self = this;
        self.dropDownForLists = $('<select style="display:none"></select>');
                        ...
                        $(self.$ulRight).find('li').each(function (i) {
                            if (!found) {
                                ...
                                if (mousePosition.x >= rect.left && mousePosition.x <= rect.right && mousePosition.y >= rect.top && mousePosition.y <= rect.bottom) {
                                    ...
                                    self.move.textB = $(this).data('text');
    ...
    this.deduplicate = function () {
        ...
        for (let i = 0; i < self.listsNr; i++) {
            ...
            for (let j = 0; j < self.data.Lists[i].list.length; j++) {
                let val = self.data.Lists[i].list[j].id;
                if (!dict[val]) {
                    dict[val] = 1;
                } else {
                    dict[val] += 1;
                    self.data.Lists[i].list[j].id += '(' + dict[val] + ')';
                }
            }
        }
    }
