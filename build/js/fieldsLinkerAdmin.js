(function ($) {
    const errMsg = "fieldsLinkerAdmin error : "
    ...
    var makeLink = function (infos) {
        ...
        linksByOrder.push({ "from": infos.offsetA, "to": infos.offsetB, "from_text": infos.textA, "to_text": infos.textB });
        linksByName.push({ "from": infos.nameA, "to": infos.nameB, "from_text": infos.textA, "to_text": infos.textB });
        draw();
    ...
    $.fn.fieldsLinkerAdmin = function (action, input) {
        ...
        if (action == "init") {
            ...
            if (input) {
                ...
                data.listA.list.forEach(function (x, i) {
                    List1.push(x);
                    var $li = $("<input class='li' value='" + x.text + "'>");
                    
                    $li
                        .appendTo($ul)
                        .attr("data-offset", i)
                        .attr("data-name", x.id)
                        .attr("data-text", x.text)
                    ...
                    $eraseIcon
                        .prependTo($li)
                        .addClass("fa fa-trash fi-remove")
                ...
                $(this).find(".FL-main .FL-left input.li").each(function (i) {
                ...
                // Computing the vertical offset of the middle of each cell.
                $(this).find(".FL-main .FL-right input.li").each(function (i) {
                    var position = $(this).position();
                    var hInner = $(this).height();
                    var hOuter = $(this).outerHeight();
                    var delta = (hOuter - hInner) / 2;
                ...
                if (effectHover == "on") {
                    ...
                    $(this).find(".FL-main .FL-left input.li").hover(function (e) {
                    ...
                    $(this).find(".FL-main .FL-right input.li").hover(function (e) {
                        if (isDisabled) return;
                ...
                $(this).find(".FL-main .FL-left input.li").on("mousedown", function (e) {
                ...
                $(this).find(".FL-main .FL-left input.li .unlink").on("click", function (e) {
                    if (isDisabled) return;
                    eraseLinkA($(this).parent().data("offset"));
                    draw();
                });

                $(this).find(".FL-main .FL-left input.li").on("mouseup", function (e) {
                    if (isDisabled) return;
                ...
                $(this).find(".FL-main .FL-right input.li").on("mouseup", function (e) {
                    if (isDisabled) return;
                ...
                $(this).find(".FL-main .FL-right input.li").on("mousemove", function (e) {
                    if (isDisabled) return;
                ...
                if (data.existingLinks) {
                    data.existingLinks.forEach(function (link) {
                        var pos = -1;
                        if (byName) {
                            var offA = List1.findIndex(x => x.id === link["from"]);
                            var offB = List2.findIndex(x => x.id === link["to"]);
                            if (offA != -1 && offB != -1) {
                                var linkWithOffset = {
                                    "from": offA,
                                    "to": offB,
